...
def border_buffer(dataframe,bufferDistance,country_name='',ISO3=''):
	try:
	...
	if dataframe1['type'].tolist()[0]=='MP':
		...
		polygon1=data_list1
		polygon=data_list1  #MultiPolygon(data_list1)
		multi_buffer_border=[]
		j=0
		for fig in data_list:
			...
			for i in range(len(fig)):
				if i==0:
					data_points=buffer_point_list([fig[i-2],fig[i],fig[1]],bufferDistance,polygon[j])
				elif i==len(fig)-1:
					data_points=buffer_point_list([fig[-2],fig[i],fig[1]],bufferDistance,polygon[j])
				else:
					data_points=buffer_point_list([fig[i-1],fig[i],fig[(i+1)]],bufferDistance,polygon[j])
				
				for i in range(len(data_points)):
					if not polygon[j].contains(Point(data_points[i])):
						buffer_border=buffer_border + [data_points[i]]
			j=j+1
...
#PROGRAM TO REMOVE BUFFER POINTS WHICH ARE AT A DISTANCE <100Km.
...
#INPUT PARAMETERS.
buffer_distance = 50
country = input('Country name as mentioned in \".dbf\" file :')
number_of_polygon_sides = 500


#Reading and extracting border data.
border = gpd.read_file('TM_WORLD_BORDERS-0.3.shp')
# Processing the data is necessary to coordinates of boundary in a usable form.
processed_data = process_data(border)

try:
	try:
		buffered_border_DATA = border_buffer(processed_data,buffer_distance,country_name=country)
	except:
		print('\nCountry name not found. Try using its ISO3 code.\n')
		country_ISO3 = input('Country ISO3 code as mentioned in \".dbf\" file :')
		buffered_border_DATA = border_buffer(processed_data,buffer_distance,ISO3=country_ISO3)
except Exception as error:
	print('\n',error)
	print('Enter correct Country name or ISO3 code from \"TM_WORLD_BORDERS-0.3.dbf\" file present in this folder.')
	quit()
...
if polygon_type == 'MP':
	for j in raw_border:
		buffered_border_x=[i[0] for i in j]
		buffered_border_y=[i[1] for i in j]
		# print(buffered_border)
		# buffered_border=Polygon(buffered_border)
		plt.fill(buffered_border_x,buffered_border_y)
		plt.plot(buffered_border_x,buffered_border_y,color='black',alpha=0.5)

	polygons=plot_polygon
	for polygon in polygons:
		border = polygon
		plt.plot(*border.exterior.xy,color='red',alpha=1)

else:
	buffered_border_poly=raw_border
	buffered_border_x=[i[0] for i in buffered_border_poly]
	buffered_border_y=[i[1] for i in buffered_border_poly]
...
if polygon_type == 'MP':
	...
	for i in range(len(raw_border)):
		...
		border=plot_polygon[i]
		plt.fill(correct_buffered_border_x,correct_buffered_border_y)
		plt.plot(correct_buffered_border_x,correct_buffered_border_y,color='black',alpha=1)
		plt.plot(*border.exterior.xy,color='red',alpha=1)
